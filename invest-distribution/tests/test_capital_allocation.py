...
def financials() -> dict:
    ...
    for scenario in ("low", "base", "high"):
        for year in forecast["forecast_years"]:
            params.append(
                {
                    "parameter_id": f"fcf_margin_{scenario}_{year}",
                    "kind": "analyst_assumption",
                    "value": 0.1,
                    "unit": "ratio",
                    "period": f"FY{year}",
                    "definition": "FCF margin",
                    "dimension": "ratio",
                    "time_basis": "annual",
                    "scenario": scenario,
                    "source_ids": [],
...
def model() -> dict:
    ...
    values = {
        ...
        "repurchases": [10, 12],
        "share_issuance": [2, 1],
        "acquisition_spend": [30, 10],
        "impairments": [0, 5],
        "internal_reinvestment": [50, 60],
        "share_count": [100, 98],
    }
    sources = []
    ...
    for year in (2024, 2025):
        source = {
            "source_id": f"filing_{year}",
            "source_type": "exchange_filing",
            "title": f"FY{year} filing",
            "publisher": "Exchange",
            "url": f"https://www.sec.gov/Archives/filing-{year}.htm",
            "published_date": f"{year + 1}-03-01",
        ...
        capture = {
            ...
            "tool_call_id": f"fixture-filing-{year}",
            "captured_date": source["accessed_date"],
            "snapshot_sha256": canonical_sha256(
                {"year": year, "document": "capital allocation table"}
            ),
            ...
            params.append(
                {
                    ...
                    "unit": unit,
                    "period": f"FY{year}",
                    "definition": measure.replace("_", " "),
                    "dimension": "quantity"
                    if measure == "share_count"
                    else ("profit" if measure == "net_income" else "cash_flow"),
                    "time_basis": "annual",
                    "scenario": "shared",
                    "currency": None if measure == "share_count" else "USD",
                    "scale": None if measure == "share_count" else "million",
                    "source_ids": [f"filing_{year}"],
                    "claim_ids": [claim_id],
                }
            )
            claims.append(
                {
                    ...
                    "locator": f"Capital allocation/{measure}",
                    "excerpt": excerpt,
...
class CapitalAllocationTests(unittest.TestCase):
    def test_calculates_dilution_flows_and_per_share_history(self) -> None:
        ...
        self.assertEqual(
            result["data"]["annual_allocation"][0]["net_repurchase_cash"], 8
        )
        self.assertEqual(
            result["data"]["annual_allocation"][0]["profit_retained_after_dividends"],
            80,
        )
        self.assertEqual(
            result["data"]["annual_allocation"][0]["net_income_per_share_unit"], 1
        )
    ...
    def test_missing_flow_does_not_default_to_zero(self) -> None:
        ...
        value["parameters"] = [
            ...
            if item["parameter_id"] != "dividends_2024"
        ]
        with self.assertRaisesRegex(
            InvestmentArtifactError, "missing allocation parameter"
        ):
    ...
    def test_historical_assumption_cannot_bypass_fact_evidence(self) -> None:
        ...
        with self.assertRaisesRegex(
            InvestmentArtifactError, "reported_fact or derived_fact"
        ):
            run_capital_allocation(financials(), value)
