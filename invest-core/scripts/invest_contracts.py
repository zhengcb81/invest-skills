...
SUPPORTED_INVEST_SUITE_VERSIONS = {
    "4.0.0",
    "4.1.0",
    "4.2.0",
    "5.0.0",
...
def revenue_runtime() -> tuple[Any, Any, Path]:
    ...
    try:
        ...
        raise InvestmentArtifactError(
            f"failed to load revenue-forecast runtime: {exc}"
        ) from exc
...
def evaluate_formula(formula: str, inputs: list[float]) -> float:
    ...
    values = [
        finite_number(value, f"formula.inputs[{index}]")
        for index, value in enumerate(inputs)
    ]
    result = _evaluate_formula_node(
        parsed, {f"x{index}": number for index, number in enumerate(values)}
    )
...
def valid_source_url(url: Any) -> bool:
    ...
    return (
        parsed.scheme == "https"
        and bool(host)
        and host not in BLOCKED_SOURCE_HOSTS
        and "." in host
    )
...
def build_scenario_manifest(
    ...
    if provided is None:
        provided = {
            ...
            "scenarios": [
                {
                    "scenario": scenario,
                    "definition": f"{scenario} scenario; assumptions remain parameter-linked in the producing module.",
                }
    ...
    payload = {
        ...
        if key != "scenario_manifest_sha256"
    }
    _fail(
        set(payload) == {"scenario_manifest_version", "source", "scenarios"},
        "scenario_manifest contains unsupported or missing fields",
    )
    _fail(
        payload["scenario_manifest_version"] == "1.0",
        "unsupported scenario_manifest_version",
    )
    _fail(
        isinstance(payload["source"], str) and payload["source"].strip(),
        "scenario_manifest.source is required",
    )
    records = payload["scenarios"]
    _fail(
        isinstance(records, list) and len(records) == len(SCENARIOS),
        "scenario_manifest must define low/base/high",
    )
    _fail(
        [item.get("scenario") for item in records if isinstance(item, dict)]
        == list(SCENARIOS),
        "scenario_manifest order must be low/base/high",
    )
    for record in records:
        _fail(
            isinstance(record, dict) and set(record) == {"scenario", "definition"},
            "invalid scenario_manifest record",
        )
        _fail(
            isinstance(record["definition"], str) and record["definition"].strip(),
            f"scenario definition is required: {record['scenario']}",
        )
    normalized = {**payload, "scenario_manifest_sha256": canonical_sha256(payload)}
    if "scenario_manifest_sha256" in provided:
        _fail(
            provided["scenario_manifest_sha256"]
            == normalized["scenario_manifest_sha256"],
            "scenario_manifest hash mismatch",
        )
    return normalized
...
GROWTH_DRIVER_SUMMARY_FIELDS = {
    "drivers",
    "unattributed_company_adjustments",
    "reconciliation",
    "rationale",
}
...
def _compact_growth_driver_summary(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "drivers": drivers,
        "unattributed_company_adjustments": analysis[
            "unattributed_company_adjustments"
        ],
...
def revenue_reference(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(publication_receipt, dict) and publication_receipt.get(
        "receipt_sha256"
    ):
    ...
    current_compliance = result[
        "schema_version"
    ] == current_revenue_schema and isinstance(workflow_receipt, dict)
    ...
    reference.update(
        {
            "revenue_compliance_status": "current_validated"
            if current_compliance
            else "legacy_read_only_validated",
    ...
    if coverage is None:
        ...
        if result["schema_version"] != "3.1":
            retained_fields.extend(
                ["measurement_basis", "measurement_periods", "measurement_rationale"]
            )
...
MANAGEMENT_TARGET_MEASUREMENT_FIELDS = {
    "measurement_basis",
    "measurement_periods",
...
def _validate_target_summary_entry(
    ...
    if status == "legacy_measurement_semantics":
        _fail(
            set(target) == MANAGEMENT_TARGET_BASE_FIELDS,
            "legacy management target summary has unexpected fields",
        )
    elif required:
        _fail(
            set(target)
            == MANAGEMENT_TARGET_BASE_FIELDS | MANAGEMENT_TARGET_MEASUREMENT_FIELDS,
            "management target summary missing measurement semantics",
        )
    else:
        valid_fields = {
            frozenset(MANAGEMENT_TARGET_BASE_FIELDS),
            frozenset(
                MANAGEMENT_TARGET_BASE_FIELDS | MANAGEMENT_TARGET_MEASUREMENT_FIELDS
            ),
        }
        _fail(
            frozenset(target) in valid_fields,
            "invalid management target summary fields",
        )
    ...
    if has_measurement_semantics:
        ...
        _fail(
            isinstance(target["measurement_periods"], list),
            f"invalid management target measurement periods: {target_id}",
        )
    _fail(
        isinstance(target["mapped_scenarios"], list)
        and set(target["mapped_scenarios"]) <= set(SCENARIOS),
        f"invalid mapped scenarios: {target_id}",
    )
    comparison = target["scenario_comparison"]
    _fail(
        isinstance(comparison, dict)
        and set(comparison) == set(target["mapped_scenarios"]),
        f"management target scenario comparison mismatch: {target_id}",
    )
    assert isinstance(comparison, dict)
    for scenario, result in comparison.items():
        _fail(
            isinstance(result, dict) and isinstance(result.get("meets_target"), bool),
            f"invalid management target attainment: {target_id}/{scenario}",
        )
...
def _validate_management_target_counts(ref: dict[str, Any], summary: list[Any]) -> None:
    ...
    _fail(
        isinstance(coverage_hash, str)
        and HASH_PATTERN.fullmatch(coverage_hash) is not None,
        "invalid management target coverage hash",
    )
    ...
    _fail(
        isinstance(counts, dict) and set(counts) == required_counts,
        "invalid management target counts",
    )
    ...
    _fail(
        ...
        "management target counts must be non-negative integers",
    )
    _fail(
        counts["targets_total"] == len(summary),
        "management target summary count mismatch",
    )


def _validate_management_target_reference(
    ref: dict[str, Any], *, required: bool
) -> None:
    status = ref.get("management_target_coverage_status")
    if status is None and not required:
        return
    _fail(
        status in {"validated", "legacy_measurement_semantics", "legacy_not_available"},
        "invalid management target coverage status",
    )
    ...
    _fail(
        ref.get("management_target_summary_sha256") == canonical_sha256(summary),
        "management target summary hash mismatch",
    )
    if status == "legacy_not_available":
        _fail(
            not summary,
            "legacy revenue reference cannot contain a management target summary",
        )
        _fail(
            ref.get("management_target_counts") is None,
            "legacy revenue reference target counts must be null",
        )
        return
...
def _validate_growth_driver_summary_entry(
    ...
    _fail(
        isinstance(driver, dict) and set(driver) == GROWTH_DRIVER_ENTRY_FIELDS,
        "invalid growth driver summary fields",
    )
    ...
    _fail(
        ...
        and driver_id.strip()
        and driver_id not in driver_ids,
        "growth driver summary IDs must be unique",
    )
    ...
    for field in (
        ...
        _fail(
            isinstance(driver[field], str) and driver[field].strip(),
            f"growth driver {driver_id}.{field} is required",
        )
    direction = driver["direction"]
    _fail(
        direction in {"growth", "headwind", "flat"},
        f"invalid growth driver direction: {driver_id}",
    )
    rank = driver["rank"]
    if direction == "flat":
        ...
        _fail(
            ...
            f"invalid growth driver rank: {driver_id}",
        )
        assert isinstance(rank, int)
        ranks[direction].append(rank)
    parameter_ids = _string_list(
        driver["parameter_ids"], f"{driver_id}.parameter_ids", allow_empty=False
    )
    segment_names = _string_list(
        driver["segment_names"], f"{driver_id}.segment_names", allow_empty=False
    )
    _fail(
        bool(parameter_ids) and bool(segment_names),
        f"growth driver mappings are required: {driver_id}",
    )
    _string_list(driver["causal_chain"], f"{driver_id}.causal_chain", allow_empty=False)
    _string_list(
        driver["leading_indicators"],
        f"{driver_id}.leading_indicators",
        allow_empty=False,
    )
    _string_list(driver["falsifiers"], f"{driver_id}.falsifiers", allow_empty=False)
    horizon = driver["horizon"]
    _fail(
        isinstance(horizon, dict) and set(horizon) == {"start_year", "end_year"},
        f"invalid growth driver horizon: {driver_id}",
    )
    ...
    _fail(
        ...
        f"invalid growth driver horizon years: {driver_id}",
    )
    increment = finite_number(
        driver["estimated_base_terminal_increment"],
        f"{driver_id}.estimated_base_terminal_increment",
    )
    share = driver["share_of_positive_driver_increment"]
    if share is not None:
        normalized_share = finite_number(
            share, f"{driver_id}.share_of_positive_driver_increment"
        )
    ...
    if direction == "growth":
        ...
        _fail(
            math.isclose(increment, 0.0, rel_tol=0, abs_tol=1e-9),
            f"flat driver increment must be zero: {driver_id}",
        )
...
def _validate_growth_driver_reference(ref: dict[str, Any], *, required: bool) -> None:
    ...
    if version is None and status is None and not required:
        return
    _fail(
        version in SUPPORTED_REVENUE_REFERENCE_SCHEMA_VERSIONS,
        "unsupported revenue reference schema version",
    )
    _fail(
        status in {"validated", "data_gap", "legacy_not_available"},
        "invalid growth driver analysis status",
    )
    summary = ref.get("growth_driver_summary")
    _fail(
        isinstance(summary, dict) and set(summary) == GROWTH_DRIVER_SUMMARY_FIELDS,
        "invalid growth driver summary",
    )
    assert isinstance(summary, dict)
    _fail(
        ref.get("growth_driver_summary_sha256") == canonical_sha256(summary),
        "growth driver summary hash mismatch",
    )
    analysis_sha256 = ref.get("growth_driver_analysis_sha256")
    if status == "legacy_not_available":
        _fail(
            analysis_sha256 is None, "legacy growth driver analysis hash must be null"
        )
    else:
        _fail(
            isinstance(analysis_sha256, str)
            and HASH_PATTERN.fullmatch(analysis_sha256) is not None,
            "invalid growth driver analysis hash",
        )
    ...
    for direction, values in ranks.items():
        _fail(
            ...
            f"growth driver {direction} ranks must be contiguous",
        )
    ...
    if status == "validated":
        ...
        _fail(
            rationale is None or isinstance(rationale, str),
            "invalid growth driver rationale",
        )
        ...
        _fail(
            ...
            "growth driver data gap requires rationale",
        )
    else:
        _fail(
            not drivers and adjustment is None and reconciliation is None,
            "legacy growth driver summary must not contain modeled values",
        )
        _fail(
            isinstance(rationale, str) and rationale.strip(),
            "legacy growth driver summary requires rationale",
        )
        return
    normalized_adjustment = finite_number(
        adjustment, "growth_driver_summary.unattributed_company_adjustments"
    )
    required_reconciliation = {
        "driver_attributed_segment_increment",
        "segment_increment_total",
        "unattributed_company_adjustments",
        "company_increment_total",
        "difference",
    }
    _fail(
        isinstance(reconciliation, dict)
        and set(reconciliation) == required_reconciliation,
        "invalid growth driver reconciliation",
    )
    ...
    _fail(
        math.isclose(
            ...
            float(reconciliation["unattributed_company_adjustments"]),
            rel_tol=0,
            abs_tol=1e-9,
        ),
        "growth driver adjustment reconciliation mismatch",
    )


def _validate_revenue_compliance_reference(
    ref: dict[str, Any], *, required: bool
) -> None:
    version = ref.get("revenue_reference_schema_version")
    if version != REVENUE_REFERENCE_SCHEMA_VERSION and not required:
        return
    _fail(
        version == REVENUE_REFERENCE_SCHEMA_VERSION,
        "current artifact requires revenue reference schema 1.2",
    )
    status = ref.get("revenue_compliance_status")
    _fail(
        status in {"current_validated", "legacy_read_only_validated"},
        "invalid revenue compliance status",
    )
    receipt_hash = ref.get("workflow_compliance_receipt_sha256")
    if status == "current_validated":
        _fail(
            ref.get("schema_version") == revenue_runtime()[0].FORECAST_SCHEMA_VERSION,
            "current revenue compliance requires current forecast schema",
        )
        _fail(
            isinstance(receipt_hash, str)
            and HASH_PATTERN.fullmatch(receipt_hash) is not None,
            "invalid revenue workflow receipt hash",
        )
    else:
        _fail(
            receipt_hash is None,
            "legacy revenue compliance cannot carry a current workflow receipt",
        )
...
def _normalize_revenue_reference(ref: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    if "management_target_coverage_status" not in normalized:
        _fail(
            normalized.get("schema_version") == "3.0",
            "revenue schema 3.1+ requires management target coverage metadata",
        )
        normalized.update(
            {
                "management_target_coverage_status": "legacy_not_available",
    ...
    if "revenue_reference_schema_version" not in normalized:
        _fail(
            normalized.get("schema_version") != "3.3",
            "revenue schema 3.3 requires growth driver reference metadata",
        )
        growth_summary = _legacy_growth_driver_summary(
            str(normalized.get("schema_version"))
        )
        normalized.update(
            {
                "revenue_reference_schema_version": REVENUE_REFERENCE_SCHEMA_VERSION,
    ...
    if normalized.get("revenue_reference_schema_version") == "1.1":
        normalized["revenue_reference_schema_version"] = (
...
def normalize_revenue_reference(ref: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    for key in ("schema_version", "engine_version", "input_sha256", "result_sha256"):
        _fail(
            ...
            f"revenue reference missing {key}",
        )
...
def adapt_revenue(
    ...
    _fail(
        isinstance(receipt, dict), "revenue result missing a valid publication_receipt"
    )
    ...
    _fail(
        receipt.get("engine_version") == result["engine_version"],
        "publication_receipt engine_version mismatch",
    )
    _fail(
        receipt.get("validated_input_sha256") == result["input_sha256"],
        "publication_receipt input hash mismatch",
    )
    _fail(
        receipt.get("formal_output_mode") == "formal",
        "invest-* must only consume formal (non-draft) revenue artifacts",
    )
    _fail(
        receipt.get("freeform_override_allowed") is False,
        "publication_receipt must forbid freeform override",
    )
    _fail(
        "output_recomputation" in receipt.get("gate_ids", []),
        "publication_receipt must certify the output_recomputation gate",
    )
    ...
    if scope == "company":
        paths = {
            scenario: {
                year: float(
                    result["consolidated_forecast"][scenario]["annual_revenue"][year]
                )
                for year in years
            }
            for scenario in SCENARIOS
        ...
        _fail(
            isinstance(segment_name, str) and segment_name.strip(),
            "segment_name is required for segment scope",
        )
        matches = [item for item in result["segments"] if item["name"] == segment_name]
        _fail(
            len(matches) == 1, f"unknown or duplicate revenue segment: {segment_name}"
        )
        segment = matches[0]
        paths = {
            scenario: {
    ...
    for position, source in enumerate(sources):
        ...
        if require_capture:
            try:
                ...
                raise InvestmentArtifactError(
                    f"invalid source capture {source_id}: {exc}"
                ) from exc
        index[source_id] = dict(source)
    return index


def _validate_parameter_period(
    parameter: dict[str, Any], as_of_date: str, parameter_id: str
) -> None:
    period = parameter.get("period")
    if parameter.get("time_basis") == "annual":
        _fail(
            isinstance(period, str) and re.fullmatch(r"FY\d{4}", period) is not None,
            f"{parameter_id}.period must use FYyyyy",
        )
    else:
        point = parse_iso_date(period, f"{parameter_id}.period")
        _fail(
            point <= parse_iso_date(as_of_date, "as_of_date"),
            f"future point-in-time parameter: {parameter_id}",
        )


def _validate_identity(
    identity: Any, *, strict: bool, allow_mixed_fiscal_year_end: bool = False
) -> dict[str, Any]:
    ...
    _fail(
        ...
        f"identity missing fields: {sorted(required - set(identity))}",
    )
    _fail(
        isinstance(identity["company_name"], str) and identity["company_name"].strip(),
        "company_name is required",
    )
    parse_iso_date(identity["as_of_date"], "as_of_date")
    _fail(
        isinstance(identity["currency"], str) and identity["currency"].strip(),
        "identity.currency is required",
    )
    _fail(
        isinstance(identity["unit"], str) and identity["unit"].strip(),
        "identity.unit is required",
    )
    _fail(
        isinstance(identity["base_year"], int)
        and not isinstance(identity["base_year"], bool),
        "base_year must be an integer",
    )
    ...
    _fail(
        ...
        "forecast_years must be integers",
    )
    if strict:
        _fail(
            re.fullmatch(r"[A-Z]{3}", identity["currency"]) is not None,
            "identity.currency must use an uppercase ISO-style code",
        )
        if allow_mixed_fiscal_year_end:
            _fail(
                identity["fiscal_year_end"] == "mixed",
                "comparison fiscal_year_end must be mixed",
            )
        else:
            _fail(
                isinstance(identity["fiscal_year_end"], str)
                and re.fullmatch(r"\d{2}-\d{2}", identity["fiscal_year_end"])
                is not None,
            ...
            try:
                ...
                raise InvestmentArtifactError(
                    "fiscal_year_end is not a valid month/day"
                ) from exc
        _fail(bool(years), "forecast_years must not be empty")
        _fail(
            years == sorted(set(years)), "forecast_years must be unique and increasing"
        )
        _fail(
            all(year > identity["base_year"] for year in years),
            "forecast_years must be after base_year",
        )
        if identity.get("company_id") is not None:
            _fail(
                isinstance(identity["company_id"], str)
                and identity["company_id"].strip(),
        ...
        if security is not None:
            ...
            _fail(
                isinstance(security.get("security_id"), str)
                and security["security_id"].strip(),
                "identity.security.security_id is required",
            )
            _fail(
                isinstance(security.get("security_type"), str)
                and security["security_type"].strip(),
                "identity.security.security_type is required",
            )
            _fail(
                finite_number(
                    security.get("units_per_security"),
                    "identity.security.units_per_security",
                )
                > 0,
                "units_per_security must be positive",
            )
    return identity
...
def parameter_by_id(
    ...
    *,
    ...
    if expected_dimensions is not None:
        _fail(
            parameter.get("dimension") in expected_dimensions,
            f"parameter dimension mismatch: {parameter_id}",
        )
    if expected_time_bases is not None:
        _fail(
            parameter.get("time_basis") in expected_time_bases,
            f"parameter time_basis mismatch: {parameter_id}",
        )
    if scenario is not None:
        _fail(
            parameter.get("scenario", "shared") in {"shared", scenario},
            f"parameter scenario mismatch: {parameter_id}/{scenario}",
        )
...
def render_parameter_template(
    template: str, scenario: str, *, year: int | None = None
) -> str:
    _fail(
        isinstance(template, str) and template.strip(), "parameter template is required"
    )
    try:
        fields = {
            field
        ...
        raise InvestmentArtifactError(
            f"invalid parameter template: {template}"
        ) from exc
    _fail(
        fields <= {"scenario", "year"},
        f"unsupported parameter template placeholder: {template}",
    )
    _fail(
        "year" not in fields or year is not None,
        f"parameter template requires year: {template}",
    )
    try:
...
def compute_security_value(
    ...
    for field in ("security_id", "security_type", "listing_currency"):
        _fail(
            ...
            f"security_bridge.{field} is required",
        )
    shares_id = render_parameter_template(
        bridge.get("diluted_share_count_parameter_template", ""), scenario
    )
    units_id = render_parameter_template(
        bridge.get("ordinary_units_per_security_parameter_template", ""), scenario
    )
    shares = parameter_by_id(
        parameters,
        shares_id,
        expected_dimensions={"quantity"},
        expected_time_bases={"point_in_time", "annual"},
        scenario=scenario,
    )
    units = parameter_by_id(
        parameters,
        units_id,
        expected_dimensions={"quantity"},
    ...
    _fail(
        share_count > 0 and units_per_security > 0,
        "security share count and conversion ratio must be positive",
    )
    fx_rate = 1.0
    if bridge["listing_currency"] == identity["currency"]:
        _fail(
            bridge.get("fx_rate_parameter_template") in (None, ""),
            "same-currency security bridge must not carry FX",
        )
    else:
        fx_id = render_parameter_template(
            bridge.get("fx_rate_parameter_template", ""), scenario
        )
        fx = parameter_by_id(
            parameters,
            fx_id,
            expected_dimensions={"currency_rate"},
            expected_time_bases={"point_in_time"},
            scenario=scenario,
        )
        _fail(
            fx.get("period") == identity["as_of_date"],
            "security FX rate must be measured at value date",
        )
        fx_rate = finite_number(fx["value"], f"{fx_id}.value")
        _fail(fx_rate > 0, "security FX rate must be positive")
    return {
        "security_id": bridge["security_id"],
        "security_type": bridge["security_type"],
        "listing_currency": bridge["listing_currency"],
        "diluted_ordinary_units": share_count,
        "ordinary_units_per_security": units_per_security,
        "fx_listing_per_model_currency": fx_rate,
        "per_security_value_current": finite_number(equity_value, "equity_value")
        * units_per_security
        / share_count
        * fx_rate,
    }
...
def _validate_parameters(
    parameters: Any, source_index: dict[str, dict[str, Any]], identity: dict[str, Any]
) -> dict[str, dict[str, Any]]:
    ...
    for position, parameter in enumerate(parameters):
        ...
        _fail(
            parameter.get("kind") in PARAMETER_KINDS,
            f"unsupported parameter kind: {parameter_id}",
        )
        ...
        _fail(
            parameter["dimension"] in PARAMETER_DIMENSIONS,
            f"unsupported parameter dimension: {parameter_id}",
        )
        _fail(
            parameter["time_basis"] in TIME_BASES,
            f"unsupported time_basis: {parameter_id}",
        )
        _validate_parameter_period(parameter, identity["as_of_date"], parameter_id)
        scenario = parameter.get("scenario", "shared")
        _fail(
            scenario in {*SCENARIOS, "shared"},
            f"invalid parameter scenario: {parameter_id}",
        )
        if parameter["dimension"] in MONETARY_DIMENSIONS:
            _fail(
                parameter.get("currency") == identity["currency"],
                f"currency mismatch: {parameter_id}",
            )
            _fail(
                parameter.get("scale") == identity["unit"],
                f"scale mismatch: {parameter_id}",
            )
        else:
            _fail(
                parameter.get("currency") in (None, ""),
                f"non-monetary parameter cannot carry currency: {parameter_id}",
            )
            _fail(
                parameter.get("scale") in (None, ""),
                f"non-monetary parameter cannot carry scale: {parameter_id}",
            )
        ...
        _fail(
            ...
            f"invalid source_ids: {parameter_id}",
        )
        _fail(
            isinstance(claim_ids, list) and len(claim_ids) == len(set(claim_ids)),
            f"invalid claim_ids: {parameter_id}",
        )
        for source_id in source_ids:
            _fail(
                source_id in source_index,
                f"unknown source_id {source_id} on {parameter_id}",
            )
        if parameter["kind"] in {"reported_fact", "management_guidance"}:
            _fail(
                bool(source_ids),
                f"{parameter['kind']} requires a source: {parameter_id}",
            )
        if parameter["kind"] in {"analyst_assumption", "scenario_stress"}:
            _fail(
                ...
                f"assumption requires rationale: {parameter_id}",
            )
        if parameter["kind"] == "derived_fact":
            _fail(
                isinstance(parameter.get("formula"), str)
                and parameter["formula"].strip(),
                f"derived parameter requires formula: {parameter_id}",
            )
            inputs = parameter.get("input_parameter_ids")
            _fail(
                isinstance(inputs, list) and inputs and len(inputs) == len(set(inputs)),
                f"derived parameter requires unique inputs: {parameter_id}",
            )
    ...
    def resolve(parameter_id: str, stack: set[str]) -> float:
        ...
        if parameter["kind"] != "derived_fact":
            ...
            return resolved[parameter_id]
        inputs = [
            resolve(item, stack | {parameter_id})
            for item in parameter["input_parameter_ids"]
        ]
        ...
        _fail(
            math.isclose(
                ...
                rel_tol=0,
                abs_tol=max(1.0, abs(calculated)) * 1e-9,
            ),
            f"derived parameter value mismatch: {parameter_id}",
        )
...
def _validate_claims(
    ...
    *,
    ...
    for position, claim in enumerate(claims):
        ...
        _fail(
            claim.get("source_id") in source_index, f"unknown claim source: {claim_id}"
        )
        _fail(
            claim.get("target_type") in TARGET_TYPES,
            f"unsupported claim target_type: {claim_id}",
        )
        _fail(
            claim.get("support_type") in SUPPORT_TYPES,
            f"unsupported claim support_type: {claim_id}",
        )
        _fail(
            isinstance(claim.get("target_id"), str) and claim["target_id"].strip(),
        ...
        _fail(
            ...
            and re.fullmatch(r"[0-9a-f]{64}", claim["content_sha256"]) is not None,
            f"invalid content hash: {claim_id}",
        )
        _fail(
            claim.get("verification_status") == "opened_and_checked",
            f"claim must be opened_and_checked: {claim_id}",
        )
        if require_capture:
        ...
        published = parse_iso_date(
            source_index[claim["source_id"]]["published_date"], "published_date"
        )
        _fail(
            published <= verified <= as_of,
            f"claim verification outside information set: {claim_id}",
        )
        if claim["target_type"] == "parameter":
            ...
            _fail(
                claim["source_id"] in parameter.get("source_ids", []),
                f"claim source not registered on parameter: {target_id}",
            )
            if claim["support_type"] == "exact_value":
                ...
                _fail(
                    ...
                    f"claim value mismatch: {target_id}",
                )
                _fail(
                    claim.get("unit") == parameter["unit"],
                    f"claim unit mismatch: {target_id}",
                )
                _fail(
                    claim.get("period") == parameter["period"],
                    f"claim period mismatch: {target_id}",
                )
    ...
    for parameter_id, parameter in parameter_index.items():
        ...
        for claim_id in parameter.get("claim_ids", []):
            ...
            _fail(
                ...
                f"claim target mismatch: {claim_id}",
            )
            linked.append(claim)
        if parameter["kind"] in {"reported_fact", "management_guidance"}:
            _fail(
                any(item["support_type"] == "exact_value" for item in linked),
                f"exact-value claim required: {parameter_id}",
            )
        if parameter["kind"] in {
            ...
            _fail(
                bool(linked), f"source-linked assumption requires claim: {parameter_id}"
            )
    return index
...
def _validate_qualitative_facts(
    ...
    for position, fact in enumerate(facts):
        ...
        _fail(
            isinstance(fact_id, str) and fact_id.strip() and fact_id not in fact_index,
            "qualitative fact IDs must be unique",
        )
        for field in ("fact_type", "statement"):
            _fail(
                isinstance(fact.get(field), str) and fact[field].strip(),
                f"{fact_id}.{field} is required",
            )
        event_date = fact.get("event_date")
        if event_date is not None:
            _fail(
                parse_iso_date(event_date, f"{fact_id}.event_date") <= as_of,
                f"future qualitative fact: {fact_id}",
            )
        claim_ids = _string_list(
            fact.get("claim_ids"), f"{fact_id}.claim_ids", allow_empty=False
        )
        for claim_id in claim_ids:
            _fail(
                claim_id in claim_index,
                f"unknown qualitative claim: {fact_id}/{claim_id}",
            )
            claim = claim_index[claim_id]
            _fail(
                claim["target_type"] == "qualitative_assertion"
                and claim["target_id"] == fact_id,
                f"qualitative claim target mismatch: {claim_id}",
            )
    ...
    for claim_id, claim in claim_index.items():
        ...
        _fail(
            claim_id in fact_index[target_id]["claim_ids"],
            f"qualitative claim is not linked back from fact: {claim_id}",
        )
    return fact_index
...
def _validate_interpretations(
    ...
    for position, item in enumerate(interpretations):
        ...
        _fail(
            isinstance(interpretation_id, str)
            and interpretation_id.strip()
            and interpretation_id not in index,
            "interpretation IDs must be unique",
        )
        _fail(
            isinstance(item.get("statement"), str) and item["statement"].strip(),
            f"{interpretation_id}.statement is required",
        )
        fact_ids = _string_list(
            item.get("fact_ids"), f"{interpretation_id}.fact_ids", allow_empty=False
        )
        contrary_ids = _string_list(
            item.get("contrary_fact_ids", []), f"{interpretation_id}.contrary_fact_ids"
        )
        _fail(
            set(fact_ids) <= set(fact_index),
            f"unknown interpretation fact: {interpretation_id}",
        )
        _fail(
            set(contrary_ids) <= set(fact_index),
            f"unknown contrary fact: {interpretation_id}",
        )
        _fail(
            not (set(fact_ids) & set(contrary_ids)),
            f"fact cannot be both supporting and contrary: {interpretation_id}",
        )
        _fail(
            item.get("confidence") in {"high", "medium", "low", "data_gap"},
            f"invalid interpretation confidence: {interpretation_id}",
        )
...
def _validate_management_data(
    ...
    identity: dict[str, Any],
    scenario_set: list[str],
    revenue_ref: dict[str, Any] | None,
    suite_version: str,
) -> None:
    expected_version = (
        "2.1" if suite_version in CURRENT_SEMANTIC_SUITE_VERSIONS else "2.0"
    )
    _fail(
        data.get("qualitative_schema_version") == expected_version,
        f"management qualitative_schema_version must be {expected_version}",
    )
    _fail(scenario_set == [], "management artifact must be non-scenario")
    facts = _validate_qualitative_facts(
        data.get("facts"), claim_index, identity["as_of_date"]
    )
    interpretations = _validate_interpretations(data.get("interpretations"), facts)
    red_flags = _string_list(
        data.get("red_flag_interpretation_ids", []), "red_flag_interpretation_ids"
    )
    _fail(
        set(red_flags) <= set(interpretations),
        "red flags must reference registered interpretations",
    )
    disconfirming = _string_list(
        data.get("disconfirming_fact_ids", []), "disconfirming_fact_ids"
    )
    _fail(
        set(disconfirming) <= set(facts),
        "disconfirming evidence must reference registered facts",
    )
    ...
    for item in commitments:
        ...
        _fail(
            ...
            and assessment_id.strip()
            and assessment_id not in commitment_ids,
            "commitment assessment IDs must be unique",
        )
        commitment_ids.add(assessment_id)
        _fail(
            item.get("commitment_fact_id") in facts,
            f"unknown commitment fact: {assessment_id}",
        )
        outcome_ids = _string_list(
            item.get("outcome_fact_ids"),
            f"{assessment_id}.outcome_fact_ids",
            allow_empty=False,
        )
        _fail(
            set(outcome_ids) <= set(facts),
            f"unknown commitment outcome fact: {assessment_id}",
        )
        _fail(
            isinstance(item.get("conclusion"), str) and item["conclusion"].strip(),
            f"commitment conclusion is required: {assessment_id}",
        )
    if expected_version == "2.0":
        return
    execution_assessments = data.get("execution_driver_assessments", [])
    _fail(
        isinstance(execution_assessments, list),
        "execution_driver_assessments must be a list",
    )
    if not execution_assessments:
        return
    _fail(
        isinstance(revenue_ref, dict),
        "execution driver assessments require revenue_forecast_ref",
    )
    assert isinstance(revenue_ref, dict)
    growth_summary = revenue_ref.get("growth_driver_summary")
    _fail(
        isinstance(growth_summary, dict),
        "execution driver assessments require a growth driver summary",
    )
    assert isinstance(growth_summary, dict)
    available_driver_ids = {item["driver_id"] for item in growth_summary["drivers"]}
    available_target_ids = {
        item["target_id"] for item in revenue_ref.get("management_target_summary", [])
    }
    assessment_ids: set[str] = set()
    required_fields = {
        "assessment_id",
        "growth_driver_ids",
        "management_target_ids",
        "input_fact_ids",
        "contrary_fact_ids",
        "status",
        "conclusion",
    ...
    for item in execution_assessments:
        _fail(
            isinstance(item, dict) and set(item) == required_fields,
            "invalid execution driver assessment fields",
        )
        ...
        _fail(
            ...
            "execution driver assessment IDs must be unique",
        )
        assert isinstance(assessment_id, str)
        assessment_ids.add(assessment_id)
        driver_ids = _string_list(
            item["growth_driver_ids"],
            f"{assessment_id}.growth_driver_ids",
            allow_empty=False,
        )
        target_ids = _string_list(
            item["management_target_ids"], f"{assessment_id}.management_target_ids"
        )
        input_fact_ids = _string_list(
            item["input_fact_ids"], f"{assessment_id}.input_fact_ids"
        )
        contrary_fact_ids = _string_list(
            item["contrary_fact_ids"], f"{assessment_id}.contrary_fact_ids"
        )
        _fail(
            set(driver_ids) <= available_driver_ids,
            f"unknown execution growth driver mapping: {assessment_id}",
        )
        _fail(
            set(target_ids) <= available_target_ids,
            f"unknown execution management target mapping: {assessment_id}",
        )
        _fail(
            set(input_fact_ids) <= set(facts) and set(contrary_fact_ids) <= set(facts),
            f"unknown execution fact mapping: {assessment_id}",
        )
        _fail(
            not (set(input_fact_ids) & set(contrary_fact_ids)),
            f"execution fact cannot be both supporting and contrary: {assessment_id}",
        )
        status = item["status"]
        _fail(
            status in {"on_track", "delayed", "off_track", "unproven", "data_gap"},
            f"invalid execution driver status: {assessment_id}",
        )
        if status in {"on_track", "delayed", "off_track"}:
            _fail(
                bool(input_fact_ids),
                f"execution status requires supporting facts: {assessment_id}",
            )
        _fail(
            isinstance(item["conclusion"], str) and item["conclusion"].strip(),
            f"execution driver conclusion is required: {assessment_id}",
        )
...
def _validate_moat_data(
    ...
    _fail(
        data.get("qualitative_schema_version") == expected_version,
        f"moat qualitative_schema_version must be {expected_version}",
    )
    facts = _validate_qualitative_facts(
        data.get("facts"), claim_index, identity["as_of_date"]
    )
    ...
    if expected_version == "2.0":
        _fail(
            revenue_ref is not None
            and registry.get("revenue_result_sha256")
            == revenue_ref.get("result_sha256"),
            "moat driver registry revenue lineage mismatch",
        )
        revenue_ids = set(
            _string_list(
                registry.get("revenue_parameter_ids", []),
                "driver_registry.revenue_parameter_ids",
            )
        )
    else:
        _fail(
            ...
            "invalid moat driver_registry fields",
        )
        growth_summary = revenue_ref.get("growth_driver_summary")
        _fail(
            isinstance(growth_summary, dict),
            "moat requires upstream growth driver summary",
        )
        assert isinstance(growth_summary, dict)
        _fail(
            registry.get("growth_driver_summary_sha256")
            == revenue_ref.get("growth_driver_summary_sha256"),
            "moat growth driver registry lineage mismatch",
        )
        available_ids = {item["driver_id"] for item in growth_summary["drivers"]}
        revenue_ids = set(
            _string_list(
                registry.get("growth_driver_ids", []),
                "driver_registry.growth_driver_ids",
            )
        )
        _fail(
            revenue_ids <= available_ids,
            "moat driver_registry contains unknown growth drivers",
        )
    financial_ids = set(
        _string_list(
            registry.get("financial_line_ids", []), "driver_registry.financial_line_ids"
        )
    )
    mechanisms = data.get("mechanisms")
    _fail(
        isinstance(mechanisms, list) and mechanisms,
        "moat mechanisms must be a non-empty list",
    )
    ...
    for position, mechanism in enumerate(mechanisms):
        ...
        _fail(
            ...
            and mechanism_id.strip()
            and mechanism_id not in mechanism_ids,
            "moat mechanism IDs must be unique",
        )
        ...
        for field in (
            ...
            _fail(
                isinstance(mechanism.get(field), str) and mechanism[field].strip(),
                f"{mechanism_id}.{field} is required",
            )
        status = mechanism.get("status")
        _fail(
            status in {"observed", "weakening", "unproven", "data_gap"},
            f"invalid moat status: {mechanism_id}",
        )
        fact_ids = _string_list(
            mechanism.get("fact_ids", []), f"{mechanism_id}.fact_ids"
        )
        contrary_ids = _string_list(
            mechanism.get("contrary_fact_ids", []), f"{mechanism_id}.contrary_fact_ids"
        )
        _fail(
            set(fact_ids) <= set(facts) and set(contrary_ids) <= set(facts),
            f"unknown moat fact mapping: {mechanism_id}",
        )
        if status in {"observed", "weakening"}:
            _fail(
                bool(fact_ids),
                f"observed moat mechanism requires facts: {mechanism_id}",
            )
        mapping_field = (
            "growth_driver_ids"
            if expected_version == "2.1"
            else "revenue_parameter_ids"
        )
        mapped_revenue = _string_list(
            mechanism.get(mapping_field, []), f"{mechanism_id}.{mapping_field}"
        )
        mapped_financial = _string_list(
            mechanism.get("financial_line_ids", []),
            f"{mechanism_id}.financial_line_ids",
        )
        _fail(
            set(mapped_revenue) <= revenue_ids,
            f"unknown moat growth driver mapping: {mechanism_id}",
        )
        _fail(
            set(mapped_financial) <= financial_ids,
            f"unknown moat financial line mapping: {mechanism_id}",
        )
        if status != "data_gap":
            _fail(
                bool(mapped_revenue or mapped_financial),
                f"moat mechanism requires a modeled driver mapping: {mechanism_id}",
            )
        durability = mechanism.get("durability_assumption")
        _fail(
            isinstance(durability, dict),
            f"durability_assumption is required: {mechanism_id}",
        )
        for field in ("horizon", "rationale"):
            _fail(
                isinstance(durability.get(field), str) and durability[field].strip(),
                f"{mechanism_id}.durability_assumption.{field} is required",
            )
        _string_list(
            mechanism.get("erosion_events", []), f"{mechanism_id}.erosion_events"
        )
        _string_list(
            mechanism.get("leading_indicators"),
            f"{mechanism_id}.leading_indicators",
            allow_empty=False,
        )
        _string_list(
            mechanism.get("falsifiers"), f"{mechanism_id}.falsifiers", allow_empty=False
        )
    disconfirming = _string_list(
        data.get("disconfirming_fact_ids", []), "moat.disconfirming_fact_ids"
    )
    _fail(
        set(disconfirming) <= set(facts),
        "moat disconfirming evidence must reference registered facts",
    )
...
def build_artifact_compliance_receipt(
    ...
    for source in sources:
        _fail(
            isinstance(source.get("capture"), dict),
            f"source capture is required: {source.get('source_id', '<unknown>')}",
        )
    ...
    receipt = {
        ...
        "contract_validator_ids": [
            "invest_core.envelope",
            "invest_core.lineage",
            "invest_core.source_capture",
            "invest_core.evidence_claims",
            "invest_core.content_hash",
        ],
        "revenue_result_sha256": None
        if revenue_forecast_ref is None
        else revenue_forecast_ref["result_sha256"],
        "upstream_artifact_sha256s": sorted(
            item["artifact_sha256"] for item in upstream_refs
        ),
    ...
    *,
    ...
    artifact_body = {
        ...
        "scenario_manifest": build_scenario_manifest(
            normalized_scenario_set, scenario_manifest
        ),
...
def _validate_artifact_envelope(artifact: dict[str, Any]) -> tuple[str, bool, str]:
    ...
    _fail(
        ...
        f"artifact missing fields: {sorted(required - set(artifact))}",
    )
    schema_version = artifact["artifact_schema_version"]
    suite_version = artifact["invest_suite_version"]
    _fail(
        schema_version in SUPPORTED_ARTIFACT_SCHEMA_VERSIONS,
        "unsupported artifact schema version",
    )
    _fail(
        suite_version in SUPPORTED_INVEST_SUITE_VERSIONS,
        "unsupported invest suite version",
    )
    strict = schema_version in {"2.0", ARTIFACT_SCHEMA_VERSION}
    current = schema_version == ARTIFACT_SCHEMA_VERSION
    if strict:
        _fail(
            "scenario_manifest" in artifact, "artifact missing field: scenario_manifest"
        )
    if current:
        _fail(
            suite_version == INVEST_SUITE_VERSION,
            "artifact schema 2.1 requires invest suite 5.2.0",
        )
        _fail(
            "compliance_receipt" in artifact,
            "artifact missing field: compliance_receipt",
        )
    elif schema_version == "2.0":
        _fail(
            suite_version in {"5.0.0", "5.1.0"},
            "artifact schema 2.0 requires invest suite 5.0.0 or 5.1.0",
        )
    else:
        _fail(
            suite_version not in {"5.0.0", "5.1.0", INVEST_SUITE_VERSION},
            "invest suite 5.x requires artifact schema 2.x",
        )
    module = artifact["module"]
    _fail(module in MODULE_REGISTRY, f"unknown module: {module}")
...
def _validate_artifact_scope(artifact: dict[str, Any], strict: bool) -> dict[str, Any]:
    ...
    identity = _validate_identity(
        artifact["identity"],
        strict=strict,
        allow_mixed_fiscal_year_end=strict
        and isinstance(raw_scope, dict)
        and raw_scope.get("type") == "comparison",
    )
    _fail(
        isinstance(raw_scope, dict)
        and raw_scope.get("type") in {"company", "segment", "comparison"},
        "invalid artifact scope",
    )
    assert isinstance(raw_scope, dict)
    if raw_scope["type"] == "segment":
        _fail(
            isinstance(raw_scope.get("name"), str) and raw_scope["name"].strip(),
            "segment scope requires name",
        )
    if raw_scope["type"] == "comparison":
        ...
        _fail(
            ...
            "comparison scope requires at least two names",
        )
...
def _validate_artifact_scenarios(artifact: dict[str, Any], strict: bool) -> list[str]:
    ...
    _fail(
        scenario_set in ([], list(SCENARIOS)),
        "scenario_set must be empty or low/base/high",
    )
    assert isinstance(scenario_set, list)
    if strict:
        expected = build_scenario_manifest(scenario_set, artifact["scenario_manifest"])
        _fail(
            artifact["scenario_manifest"] == expected,
            "scenario_manifest normalization mismatch",
        )
    return scenario_set


def _validate_artifact_revenue_reference(
    module: str, ref: Any, strict: bool, suite_version: str
) -> None:
    ...
    _validate_growth_driver_reference(
        ref, required=suite_version in CURRENT_SEMANTIC_SUITE_VERSIONS
    )
    _validate_revenue_compliance_reference(
        ref, required=suite_version == INVEST_SUITE_VERSION
    )
...
def _validate_artifact_upstream(module: str, upstream: Any) -> None:
    ...
    for item in upstream:
        _fail(
            ...
            and set(item) == {"module", "artifact_id", "artifact_sha256"},
    ...
    for required_module in cast(tuple[str, ...], MODULE_REGISTRY[module]["upstream"]):
        _fail(
            any(item["module"] == required_module for item in upstream),
            f"{module} requires upstream module: {required_module}",
        )
...
def _validate_artifact_content_hashes(artifact: dict[str, Any], strict: bool) -> None:
    if strict:
        _fail(
            ...
            and HASH_PATTERN.fullmatch(artifact["artifact_id"]) is not None,
            "invalid artifact_id",
        )
        _fail(
            isinstance(artifact["artifact_sha256"], str)
            and HASH_PATTERN.fullmatch(artifact["artifact_sha256"]) is not None,
            "invalid artifact_sha256",
        )
    ...
    _fail(
        artifact["artifact_sha256"] == canonical_sha256(payload),
...
def validate_artifact(artifact: dict[str, Any]) -> None:
    ...
    _fail(
        isinstance(artifact["limitations"], list)
        and all(
            isinstance(item, str) and item.strip() for item in artifact["limitations"]
        ),
        "limitations must contain strings",
    )
    _fail(
        len(artifact["limitations"]) == len(set(artifact["limitations"])),
    ...
    source_index = _validate_sources(
        artifact["sources"], identity["as_of_date"], require_capture=current
    )
    parameter_index = _validate_parameters(
        artifact["parameters"], source_index, identity
    )
    ...
    if strict and module == "management":
        _validate_management_data(
            artifact["data"], claim_index, identity, scenario_set, ref, suite_version
        )
    ...
    if current:
        ...
        _fail(
            artifact["compliance_receipt"] == expected_receipt,
            "artifact compliance receipt mismatch",
        )
...
def finalize_draft(draft: dict[str, Any]) -> dict[str, Any]:
    ...
    _fail(
        required <= set(draft),
        f"artifact draft missing fields: {sorted(required - set(draft))}",
    )
    _fail(
        draft["module"] in QUALITATIVE_DRAFT_MODULES,
        "finalize-draft is restricted to qualitative management or moat artifacts",
    )
    return create_artifact(
        ...
        evidence_claims=draft.get("evidence_claims", []),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate investment artifacts and adapt revenue forecasts"
    )
    ...
    adapt_parser.add_argument(
        "--scope", choices=("company", "segment"), default="company"
    )
