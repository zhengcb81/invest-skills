...
if _REVENUE_FORECAST is None:
    ...
    for candidate in (here / "revenue-forecast", here.parent / "revenue-forecast", here.parent.parent / "revenue-forecast"):
        if (candidate / "scripts").is_dir():
            _REVENUE_FORECAST = candidate
            break
if _REVENUE_FORECAST is None:
...
def _make_heterogeneous_forecast() -> dict:
    ...
    for name, spec in specs.items():
        ...
        for scenario, multiplier in (("low", 0.85), ("base", 1.0), ("high", 1.15)):
            driver_ids = {}
            for position, (driver, (dimension, base_value)) in enumerate(spec["drivers"].items()):
                value = base_value * (multiplier if position == 0 else 1.0)
                ids = [
                    add_parameter(
                        data, name.lower() + "_" + driver + "_" + scenario + "_" + str(year),
                        value, year, scenario, dimension=dimension,
                    )
    ...
    reported_param = next(
        item for item in data["parameters"] if item["parameter_id"] == data["reported_total_revenue_parameter_id"]
    )
    reported_param["value"] = reported
    base_year_hist = next(item for item in data["historical_revenue"] if item["year"] == data["base_year"])
    ...
    for scenario in ("low", "base", "high"):
        cap_params[scenario] = [
            add_parameter(data, "shared_cap_" + scenario + "_" + str(year), 150, year, scenario, dimension="revenue")
            for year in data["forecast_years"]
        ]
    ...
    for scenario in ("low", "base", "high"):
        elim_params[scenario] = [
            add_parameter(data, "services_elim_" + scenario + "_" + str(year), -5, year, scenario, dimension="revenue")
            for year in data["forecast_years"]
        ]
    ...
    foundation_record = next(item for item in data["research_coverage"] if item["dimension"] == "company_foundation")
