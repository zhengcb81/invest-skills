...
4. Select one explicit valuation method or an explicitly weighted method result for each segment.
5. Register ownership percentages and signed company-level adjustments as parameters.
6. Calculate low/base/high ownership-adjusted segment values and the company bridge.
...
- Apply a conglomerate discount only as an explicit signed adjustment with an assumption and sensitivity; never infer it from the number of segments.
